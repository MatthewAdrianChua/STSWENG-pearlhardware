...
describe('Refund Controller', () => {
    ...
    describe('getOrderDetails', () => {
      ...
      describe('getAdminCategory', () => {
        ...
        it('should render adminOrders page with valid parameters', async () => {
          // Mock Order.find to return some dummy data
          Order.find = jest.fn().mockResolvedValue([{ _id: 'mockOrderID', firstName: 'John', lastName: 'Doe', email: 'john@example.com', date: new Date(), status: 'succeeded', amount: 100, paymongoID: 'mockPaymongoID', isCancelled: false }]);
      
          // Mock User.findById to return authorized user
          //User.findById = jest.fn().mockResolvedValue({ isAuthorized: true });

          jest.spyOn(User, 'findById').mockResolvedValue({ isAuthorized: true })
      
          // Call the function under test
          await orderController.getAdminCategory(req, res);
